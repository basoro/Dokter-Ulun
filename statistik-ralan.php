            ...
            <div class="row clearfix">
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <div class="card">
						...
						$penyakit=array();
						$kd_penyakit=array();
                        $sql = "SELECT MONTHNAME (tgl_registrasi) as bulan, COUNT(DISTINCT  no_rawat) as jumlah FROM reg_periksa WHERE kd_dokter = '{$_SESSION['username']}' AND tgl_registrasi LIKE '$tahun%' GROUP BY MONTH(tgl_registrasi)";
						$hasil=query($sql);
						while ($data = fetch_array ($hasil)){
                            $ralan_jumlah[]=intval($data['jumlah']);
                            $ralan_bulan[]=$data['bulan'];
                        }
						?>
                                <div id="pasien_ralan">
                                </div>
...
<?php include_once ('layout/footer.php'); ?>
	...
	<script type="text/javascript">
        Highcharts.chart('pasien_ralan', {
		    ...
		    title: {
			    text: 'Pasien Rawat Jalan <?php echo ucwords(strtolower($dataGet['0'])); ?>'
			},
			subtitle: {
				text: <?=json_encode($tahun);?>
			},
		    xAxis: {
		        categories: <?=json_encode($ralan_bulan);?> ,
